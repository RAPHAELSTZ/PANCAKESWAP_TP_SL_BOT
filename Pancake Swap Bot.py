...
SWAP_FROM = Entry()
SWAP_FROM.grid(column=1, row=2, padx=5)
...
variable.set("one") # default valu

w = OptionMenu(tk, variable, "one", "two", "three")
w.grid(column=1, row=4)

Label(text="Convert to .exe").grid(column=0, row=5, sticky="w")

Button(text="Convert", command=convert).grid(column=1, row=5, pady=5)
